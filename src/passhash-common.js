...
import { b64_hmac_sha1 } from "./passhash-sha1"


export var PassHashCommon = {

  bump: function(tag) {
    var newtag = tag.replace(/^[ \t]*(.*)[ \t]*$/, "$1")
    if (newtag) {
      var splitTag = newtag.match(/^(.*):([0-9]+)?$/)

      if   (splitTag == null || splitTag.length < 3) newtag += ":1"
      else newtag = splitTag[1] + ":" + (parseInt(splitTag[2]) + 1)
    }
